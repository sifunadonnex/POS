...
- Catalogue: product/category maintenance, exact integer-minor-unit prices, barcodes, archive/history and atomic CSV import are implemented and PostgreSQL-tested. Units are `each`, `pack`, `kg` and `l`; later stock/checkout quantities use steps of 1 for `each`/`pack` and 0.001 for `kg`/`l`. Fractional sale-total rounding still needs explicit examples before checkout.
