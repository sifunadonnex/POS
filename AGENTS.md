...
- Backend: NestJS, TypeScript and PostgreSQL via `pg`, with explicit SQL migrations managed by `node-pg-migrate`. PostgreSQL 18 is installed and running locally. On 16 September 2026 all migrations were applied to the development database and the isolated `_test` integration suites passed. See `docs/BACKEND_SETUP.md`; hosted PostgreSQL remains unverified.
- Authentication: Better Auth 1.7.4 provides email/password login, database sessions and server-side manager/cashier roles. Recovery, required verification, manager MFA, staff administration, audit and inactivity locking are implemented; PostgreSQL integration and local manager/cashier password login were verified on 16 September 2026. SMTP delivery and full interactive browser verification remain deferred. Read `docs/LOCAL_AUTH_SETUP.md` and `docs/SECURITY_VERIFICATION.md`. Use relevant Better Auth skills and version-matched official documentation before changing authentication. Public registration is blocked; email uses the user's existing SMTP mailbox, configured privately.
- Catalogue: product/category maintenance, exact integer-minor-unit prices, barcodes, archive/history and atomic CSV import are implemented and PostgreSQL-tested. Units are `each`, `pack`, `kg` and `l`; stock/checkout quantities use steps of 1 for `each`/`pack` and 0.001 for `kg`/`l`. Fractional checkout lines round to the nearest minor unit with exact halves up; repeated partial customer refunds use cumulative rounded allocation so a full return equals the original sale line. Supplier-cost rounding remains undecided.
- Payments: cash checkout is active. Card/M-Pesa use a durable payment-attempt/event/reconciliation foundation with exact amount matching and idempotency, but the runtime gateway is deliberately disabled and the UI methods remain unavailable until an approved provider, credentials and callback/reconciliation workflow are configured and verified.
