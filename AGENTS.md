...
- Backend: NestJS, TypeScript and PostgreSQL via `pg`, with explicit SQL migrations managed by `node-pg-migrate`. User confirmed on 15 September 2026 that PostgreSQL is not installed on the current machine. Continue implementation and database-independent checks; defer database installation, migrations, account provisioning and real database tests until PostgreSQL is available. Earlier PostgreSQL 18.6 evidence is historical, not current-machine verification. See `docs/BACKEND_SETUP.md`.
- Authentication: Better Auth 1.7.4 provides email/password login, database sessions and server-side manager/cashier roles. Recovery, required verification, manager MFA, staff administration, audit and inactivity locking are implemented in source; PostgreSQL, SMTP and browser verification remain deferred. Read `docs/LOCAL_AUTH_SETUP.md` and `docs/SECURITY_VERIFICATION.md`. Use relevant Better Auth skills and version-matched official documentation before changing authentication. Public registration is blocked; email uses the user's existing SMTP mailbox, configured privately.
- User-directed scope (15 September 2026): finish login/security work before the full POS backlog. The later first catalogue/checkout must include weight and volume sales; exact unit precision and rounding examples remain to be defined.
