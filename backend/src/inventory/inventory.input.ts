...
export function quantityInput(
  ...
  if (unit === 'each' || unit === 'pack') {
    if (!Number.isInteger(numeric)) {
      throw new BadRequestException(`${field} must be a whole number for ${unit}`);
    }
    return numeric;
  }
  ...
  if (!Number.isFinite(minor) || Math.abs(minor - numeric * 1000) > 1e-8) {
    throw new BadRequestException(`${field} must have a precision of 0.001 for ${unit}`);
  }
  return numeric;
}

export function requestInput(value: unknown): { requestId: string; reason: string } {
