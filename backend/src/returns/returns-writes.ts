    ...
    try {
      ...
      if (receipt) {
        if (receipt.actor_id !== actor.userId || receipt.fingerprint !== fingerprint) {
          throw new ConflictException('This request ID was already used for a different return');
        }
      ...
      if (error && typeof error === 'object' && 'code' in error) {
        if (error.code === '23505') {
          throw new ConflictException('This return request is already committed. Reload and retry.');
        }
      }
      throw new ServiceUnavailableException('The return could not be completed. Retry the same request to check its outcome.');
    } finally {
      client?.release();
    }
  }
