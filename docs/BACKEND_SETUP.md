...
**Current machine (verified, 16 September 2026): PostgreSQL 18 is installed and the `postgresql-x64-18` service is running.** Separate local development/test databases and ignored environment files are configured. All migrations and the real integration suites pass. Connected browser, SMTP, backup/restore and hosted testing remain separate gates.
...
## Current local database setup and verification

The local PostgreSQL server runs at `127.0.0.1:5432` as Windows service `postgresql-x64-18`. Two databases use separate, randomly generated credentials:
...
Verified on 16 September 2026: all four migrations are applied to `pay_and_go_dev`; migration up/repeated up/down/reapply works in disposable test schemas; all 15 database integration tests pass; development manager/cashier password sessions reach `/api/identity/me` and were signed out. Hosting, SMTP, backup/restore, browser usability and phone-access proof remain deferred. Local success does not establish HostPinnacle readiness.
